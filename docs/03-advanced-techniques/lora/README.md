...
**대상** 모델 전체를 새로 학습하지 않고 특정 화풍만 추가하고 싶은 분 · **사전 이해** 기본 이미지 생성 경험 (Part 2 수료 권장) · **시간** 20분
...
화가에 비유하면, 기존 모델은 여러 그림을 그릴 줄 아는 화가이고 LoRA는 그 화가가 들은 특정 화풍 단기 수업입니다. 수업을 들었다고 원래 실력이 사라지지는 않습니다.

### 왜 LoRA를 쓰는가

원하는 화풍을 얻는 방법은 두 가지입니다. **모델 전체를 다시 학습하거나(전체 미세조정), 얹는 파일 하나만 만드는 것(LoRA)**입니다. 아래는 **만드는 사람 입장**에서의 차이입니다. 받아서 쓰는 입장에서 중요한 것은 마지막 두 줄입니다.
...
| **쓰는 방법** | 체크포인트 자체를 교체 | 쓰던 체크포인트에 **얹어서** 사용 |

받는 쪽에서 얻는 이점은 분명합니다. **파일이 작고, 쓰던 모델을 바꾸지 않아도 되며, 마음에 안 들면 노드 하나만 빼면 원래대로 돌아옵니다.**
...
출력된 MODEL은 KSampler로, CLIP은 CLIP Text Encode로 각각 이어 줍니다. 연결 그림은 아래 3번에 있습니다.
...
**5. 실행**
`Queue Prompt`를 누르고, 프롬프트에 트리거 워드가 들어갔는지 확인합니다.

---

## 트리거 워드

LoRA 효과가 보이지 않을 때 가장 흔한 원인은 강도가 아니라 **트리거 워드(trigger word)** 누락입니다.
...
- 예시 이미지가 있다면 그 프롬프트를 그대로 한 번 재현해 보는 것이 가장 빠른 확인 방법입니다.
...
처음에는 **두 값을 같게** 두고 함께 움직이세요. 대부분의 LoRA는 이 사용법을 기준으로 배포됩니다. 화풍은 유지하되 프롬프트 반영만 조절하고 싶을 때처럼 목적이 분명해진 다음에 값을 분리합니다.
...
| 0.6-0.8 | 중간 효과 | **대부분의 경우 여기서 시작** |
| 0.9-1.0 | 강한 효과 | 스타일을 뚜렷하게 낼 때 |
| 1.0 초과 | 매우 강함 | 특수한 경우만 |

### 조절 팁

0.3 이하에서는 스타일이 거의 보이지 않고, 1.0을 넘기면 스타일이 과해지면서 디테일이 뭉개지고 품질이 떨어집니다. 0.6~0.8 구간이 스타일과 품질의 균형점이라 여기서 출발하는 편이 안전합니다.
...
- 쓰는 LoRA에 트리거 워드가 있는지 확인하고 프롬프트에 반영했다.
...
[홈](../../README.md) · [LoRA 조합과 선택](combining.md)
