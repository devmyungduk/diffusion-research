...
> 제어 워크플로우에서 쓰는 Flux 모델(Dev/Fill), 정밀도(FP16/FP8), 양자화(GGUF) 선택 기준을 정리합니다.
...
**대상** Flux를 쓰려는데 파일 선택에서 막힌 사용자 · **사전 이해** 자신의 GPU VRAM 용량 · **시간** 10분
...
**참고 측정값 (RTX 4080 Super, 50 steps 1회 측정):**
```
FP16: 94.77초
FP8:  54.77초
차이:  약 42% 단축
...
한 환경에서 잰 값입니다. GPU·해상도·steps·다른 노드 구성에 따라 달라지므로 자기 환경에서 직접 재보세요.
...
!!! warning "Flux.1 Fill로는 새 이미지를 만들 수 없습니다"
    Fill은 기존 이미지의 일부를 채우는 전용 모델입니다. 프롬프트만으로 처음부터 그림을 만들려면 Dev를 쓰세요.
...
**실용 조언:** FP8의 품질 저하는 대체로 크지 않으며 upscaler·detailer로 보완할 수 있습니다.
...
GGUF 파일은 예외적으로 `ComfyUI/models/unet/`에 둡니다. 자세한 배치는 [Flux Quick Reference](../../02-models/flux/quick-reference.md#필수-파일-위치)를 참고하세요.
