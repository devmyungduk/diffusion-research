...
마스크는 **위쪽 경로로만** 갑니다. `DifferentialDiffusion`에는 마스크 입력이 없고, 아래쪽 MODEL 라인에서 "마스크의 회색조를 어떻게 읽을지"만 바꿉니다([Differential Diffusion](differential-diffusion.md#3-파이프라인-통합)).
