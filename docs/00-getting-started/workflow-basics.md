[홈](../README.md) · [시작하기](README.md) · [이전: 빠른 시작](quick-start.md) · [다음: 프롬프트와 CLIP](prompt-basics.md)
...
**이럴 때 읽으세요** 첫 이미지는 만들었고 각 노드의 의미가 궁금할 때. 프롬프트 쪽은 [다음 장](prompt-basics.md)에서 다룹니다.
</div>
...
4. **KSampler** — MODEL + 조건 + Latent을 받아 노이즈 제거
5. **VAE Decode** — 완성된 Latent을 이미지로 복원
...
- **CLIP 한 갈래가 두 노드로** 나뉩니다. Positive와 Negative가 같은 텍스트 인코더를 씁니다.
- **VAE는 KSampler를 거치지 않습니다.** Load Checkpoint에서 VAE Decode로 곧장 갑니다.
- **KSampler에 선으로 들어가는 것은 4개**입니다. 나머지 설정값은 노드 안에서 직접 입력합니다.
...
화가에게 주는 지시의 강도로 바꿔 생각하면, 낮은 값은 "자유롭게 그려주세요", 중간은 "이렇게 그려주세요", 높은 값은 "정확히 이렇게만 그려주세요"에 가깝습니다.
...
| **ddim** | 결정적 | 초기 Diffusion 계열 샘플러입니다 | 다른 도구와 결과를 맞출 때 |
...
스텝을 늘려도 개선이 멈추는 지점은 Sampler와 Scheduler 조합에 따라 다릅니다. Seed를 고정하고 Steps만 바꿔 직접 확인하는 편이 정확합니다.
...
ComfyUI에서 Sampler와 Scheduler는 **별개 항목**입니다. 다른 도구에서 쓰는 `dpmpp_2m_karras` 같은 이름은 두 값을 붙여 쓴 표기이므로, ComfyUI에서는 `sampler_name`에 `dpmpp_2m`, `scheduler`에 `karras`를 각각 지정합니다.
...
원하는 결과를 정한 뒤 **어떤 변수부터 만질지 고르는 순서**는 [첫 워크플로우 직접 만들기](first-workflow.md#목표를-정해-값-고르기)에서 다룹니다.
