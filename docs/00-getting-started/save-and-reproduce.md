...
- 따라서 원본 보관 기준은 `.json` 저장이고, PNG 복원은 편의 기능으로 사용합니다.
...
1. `control_after_generate`를 `fixed`로 두고 한 장 만듭니다.
2. 워크플로우를 저장합니다.
...
`.json` 파일만으로 부족한 경우가 있습니다. 모델 파일 이름이 같아도 내용이 다른 버전이 유통되고, 시간이 지나면 어떤 파일을 사용했는지 기억나지 않습니다.
...
아래 형식을 복사해 워크플로우 파일과 같은 폴더에 남깁니다. `.json`에 담기지 않는 항목이 여기에 들어갑니다.
