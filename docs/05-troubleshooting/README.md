...
!!! warning "선은 붙는데 단색 화면만 나오는 경우"
    `Empty Latent Image`를 KSampler를 건너뛰고 `VAE Decode`에 바로 연결하면 **둘 다 `LATENT` 타입이라 선이 붙습니다.** 이때 KSampler는 출력이 어디에도 사용되지 않아 실행 자체를 건너뛰고, 0으로 채워진 빈 Latent가 그대로 디코딩되어 **아무 형태도 없는 단색 화면**이 나옵니다. `VAE Decode`의 `samples`가 **KSampler의 출력에서 오는지** 확인하세요.
...
값과 반영 강도는 비례하지 않습니다. 값의 의미와 실용 상한은 [프롬프트 가중치](../03-advanced-techniques/prompt-weighting.md)에 있습니다.
...
첫 장을 만들 때의 시작값은 [빠른 시작](../00-getting-started/quick-start.md#4-첫-실행값-넣기)의 표(SD 1.5 `7.0`, SDXL `6.0`)입니다. 위 범위는 결과를 보고 조정할 때 사용합니다.
...
??? warning "세 가지 오류 메시지 — 원인과 조치 펼치기"
...
1. 커스텀 노드를 모두 끈 채로 시작합니다. 데스크톱 앱은 설정 메뉴에서 끄고 시작할 수 있고, 직접 설치한 경우 `python main.py --disable-all-custom-nodes`로 실행합니다.
2. 이 상태에서 증상이 사라지면 원인은 커스텀 노드입니다. 그대로면 커스텀 노드가 원인이 아니므로 위 1~5절에서 원인을 찾습니다.
3. 원인이 커스텀 노드라면 절반씩 켜 가며 범위를 좁힙니다. 하나씩 켜는 것보다 재시작 횟수가 적습니다.
4. 문제를 일으키는 노드를 찾으면 갱신하거나, 같은 기능의 다른 노드로 바꾸거나, 뺍니다.
...
- [워크플로우 저장과 재현](../00-getting-started/save-and-reproduce.md) — 어떤 값으로 만든 결과인지 남기는 방법
