...
**대상 독자:** Flux 모델을 본격적으로 활용하고 싶은 중급자 ~ 고급자
**사전 이해:** SD 기본 사용법 (Load Checkpoint, KSampler, VAE 개념)
**예상 소요 시간:** 40분

**핵심 내용:**
- Flux와 기존 SD 모델의 근본적 차이 (Transformer 아키텍처)
- Flux 전용 파이프라인 구축법 (Dual CLIP, Load Diffusion Model)
- Flux Redux, ControlNet 등 최신 기술 활용
