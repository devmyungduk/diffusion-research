...
> Flux 파이프라인에서 자주 찾는 값들을 한곳에 모았습니다. 개념 설명은 [Flux 모델 가이드](README.md), 실전 구현은 [FLUX.1 작업 선택과 Redux 실습](flux-practical.md)을 참고하세요.
...
GGUF만 `unet/`을 사용하는 예외입니다. `.safetensors` 형식의 Flux 본체는 위 표대로 `diffusion_models/`에 둡니다.
...
??? note "시스템 요구사항 표 펼치기"
    | 구성요소 | FP8로 돌릴 때 | FP16으로 돌릴 때 |
    |:---|:---|:---|
    | **VRAM** | 12 GB | 24 GB |
    | **RAM** | 16 GB | 32 GB 이상 (T5XXL FP16이 약 9.8GB) |
    | **Storage** | 40 GB | 100 GB |

    GPU는 제품명이 아니라 **VRAM 용량**으로 판단합니다. 12GB 미만이면 [GGUF 양자화](../../03-advanced-techniques/controlnet/flux-model-variants.md#5-양자화-옵션)를 확인하세요.
