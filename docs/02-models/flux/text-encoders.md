...
T5XXL은 프롬프트 문장을 받아 `[seq_len × 4096]` 크기의 임베딩으로 바꾸고, Flux 모델은 이 임베딩을 생성 과정의 지침으로 씁니다. 문장이 길고 문맥이 복잡해도 비교적 잘 처리하는 것이 이 인코더의 장점입니다.
...
> **기억법**: CLIP은 종이 클립처럼 텍스트와 이미지를 하나로 집어 둡니다.
...
**질문:** 텍스트 벡터가 Latent으로 변환되는 걸까요?

**답:** 아닙니다. 벡터는 Latent이 만들어지는 **방향을 안내**할 뿐, 그 자체가 Latent이 되지는 않습니다.

| 설명 | 판정 | 이유 |
|:---:|:---:|:---:|
| 벡터가 Latent으로 변환된다 | 틀림 | 두 데이터는 서로 변환되지 않습니다 |
...
GPS는 길을 알려줄 뿐, GPS 자체가 자동차가 되지는 않습니다.
```
...
- 텍스트 벡터가 Latent으로 변환되는 것이 아니라 생성 방향을 안내한다는 점을 설명할 수 있다.
